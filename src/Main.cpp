...
#define X_LEFT_MOTOR_DIR 5
#define X_LEFT_MOTOR_STEP 2
#define Y_RIGHT_MOTOR_DIR 6
#define Y_RIGHT_MOTOR_STEP 3
#define Z_LEFT_MOTOR_DIR 7
#define Z_LEFT_MOTOR_STEP 4
#define A_RIGHT_MOTOR_DIR 13
#define A_RIGHT_MOTOR_STEP 12
#define ENABLE_PIN 8

#define VOLTAGE_CHECK_PIN A0  // abort pin
#define RX_PIN A1             // hold pin
#define TX_PIN A2             // resume pin
...
void setup() {
  ...
  pinMode(X_LEFT_MOTOR_STEP, OUTPUT);
  pinMode(X_LEFT_MOTOR_DIR, OUTPUT);
  pinMode(Y_RIGHT_MOTOR_STEP, OUTPUT);
  pinMode(Y_RIGHT_MOTOR_DIR, OUTPUT);
  pinMode(Z_LEFT_MOTOR_STEP, OUTPUT);
  pinMode(Z_LEFT_MOTOR_DIR, OUTPUT);
  pinMode(A_RIGHT_MOTOR_STEP, OUTPUT);
...
void loop() {
  ...
  // 1250 / 1023 = 1.222V per interval
  // The variable battery_voltage holds 1050 if the battery voltage is 10.5V.
  battery_voltage = (analogRead(VOLTAGE_CHECK_PIN) * 1.222) + 85;  // 85 diode consumed voltage
  if (0) {
    if (battery_voltage > 800 && battery_voltage < 1050) {  // If batteryvoltage is below 10.5V and higher than 8.0V
      digitalWrite(LED_BUILTIN, HIGH);                      // Turn on the led if battery voltage is too low
  ...
  if (accelerometer_data_raw < -8200) accelerometer_data_raw = -8200;     // Prevent division by zero by limiting the acc data to +/-8200;
  angle_acc = asin((float)accelerometer_data_raw / 8200.0) * (180 / PI);  // Calculate the current angle according to the accelerometer
...
ISR(TIMER2_COMPA_vect) {
  ...
  if (throttle_counter_left_motor > throttle_left_motor_memory) {  // If the number of loops is larger then the throttle_left_motor_memory variable
    ...
    if (throttle_left_motor_memory < 0) {                          // If the throttle_left_motor_memory is negative
      digitalWrite(X_LEFT_MOTOR_DIR, LOW);                         // Set output 3 low to reverse the direction of the stepper controller
      throttle_left_motor_memory *= -1;                            // Invert the throttle_left_motor_memory variable
    } else {
      digitalWrite(X_LEFT_MOTOR_DIR, HIGH);  // Set output 3 high for a forward direction of the stepper motor
    }
  } else if (throttle_counter_left_motor == 1) {
    digitalWrite(X_LEFT_MOTOR_STEP, HIGH);  // Set output 2 high to create a pulse for the stepper controller
  } else if (throttle_counter_left_motor == 2) {
    digitalWrite(X_LEFT_MOTOR_STEP, LOW);  // Set output 2 low because the pulse only has to last for 20us
  }
  ...
  if (throttle_counter_right_motor > throttle_right_motor_memory) {  // If the number of loops is larger then the throttle_right_motor_memory variable
    ...
    if (throttle_right_motor_memory < 0) {                           // If the throttle_right_motor_memory is negative
      digitalWrite(Y_RIGHT_MOTOR_DIR, LOW);                          // Set output 5 low to reverse the direction of the stepper controller
      throttle_right_motor_memory *= -1;                             // Invert the throttle_right_motor_memory variable
    } else {
      digitalWrite(Y_RIGHT_MOTOR_DIR, HIGH);  // Set output 5 high for a forward direction of the stepper motor
    }
  } else if (throttle_counter_right_motor == 1) {
    digitalWrite(Y_RIGHT_MOTOR_STEP, HIGH);  // Set output 4 high to create a pulse for the stepper controller
